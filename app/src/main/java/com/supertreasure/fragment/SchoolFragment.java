...
import com.supertreasure.home.ActivityMoodInPut;
import com.supertreasure.home.SchoolAdapter;
...
public class SchoolFragment extends BaseFragment {
    ...
    private boolean mHasLoadedOnce;//说明页面已经打开
    ...
    public View initView() {
        ...
        View view = View.inflate(mActivity, R.layout.layout_school, null);
   ...
   /* @Subscribe
    public void onEventAsync(InitBanner1FromServer event){
   ...
   public void onCreate(Bundle savedInstanceState) {
    ...
    public void initData() {
        ...
        //moodList = DBTool.getInstance(context).queryMoodList(context);
        //----------------------------------------------------------------------
        AbSharedUtil.putInt(context, Config.SchoolInPage, 1);
        ...
        //int referenceMoodId = tool.queryMoodCurrentNewestId(context);
        referenceMoodId =  greenUtils.queryMoodCurrentNewestId(context,Config.DATABASE_MOOD_TYPE_IN);
        //moodList = tool.queryMoodList(context,currentPage,referenceMoodId);
        ...
        TheApp.mAbHttpUtil.post(url, new AbStringHttpResponseListener() {
            ...
            public void onSuccess(int statusCode, String content) {
                ...
                if(nowTime.getStatus().equals(Config.SUCCESS)){
                    AbSharedUtil.putString(context,Config.SchoolInTime,nowTime.getNowtime());
                    AbSharedUtil.putInt(context, Config.SchoolInPage, 1);
                    getMoodList();
                }
            }
    ...
    private void getMoodList() {
        ...
        String url = Config.getAPI(Config.APIMoodIn);
        ...
        params.put(Config.Page, AbSharedUtil.getInt(context, Config.SchoolInPage));
        params.put(Config.Rows, Config.RowNumber);
        params.put(Config.BeginTime, AbSharedUtil.getString(context, Config.SchoolInTime));

        TheApp.mAbHttpUtil.post(url, params, new AbStringHttpResponseListener() {
            @Override
            public void onSuccess(int statusCode, String content) {
                //Logger.json("response content", content);
                if (HttpResultUtil.handleUnloginErrorStatus(content, RefleshSchoolInMoodFragment.getInstance(), SchoolFragment.this.getActivity())) {
                    return;
                }
                // Log.i("result",content);
                Gson gson = new Gson();
                MoodBean moodBean = gson.fromJson(content, MoodBean.class);
                //Log.w("getPublishtime",moodBean.getList().get(0).getPublishtime());
                // Log.w("haha",content);
                if (moodBean.getStatus().equals(Config.SUCCESS)) {
                    if (isPullToRefresh) {
                        if (moodBean.getList() != null && moodBean.getList().size() > 0)
                            greenUtils.deleteAllMoodByType(Config.DATABASE_MOOD_TYPE_IN);
                    ...
                    /*for (MoodBean.Mood mood : moodBean.getList()){
                    ...
                    for (Mood mood : moodBean.getList()) {
                        greenUtils.insertOrReplaceMood(mood,Config.DATABASE_MOOD_TYPE_IN);
                    }
                    //tool.insertMoodTable(context,moodBean.getList());
                    //Toast.makeText(context,AbSharedUtil.getString(context,),Toast.LENGTH_SHORT).show();
                    AbSharedUtil.putInt(context, Config.SchoolInPage, AbSharedUtil.getInt(context, Config.SchoolInPage) + 1);
                }
            }
    ...
    public void setListener() {
        ...
        adapter.setOnClickListener(new SchoolAdapter.OnItemClickListener() {
            ...
            public void ItemClickListener(View ll_message, Mood mood, int position) {
                ...
                bundlePeople.putString("belongschool", mood.getBelongschool());
                bundlePeople.putString("sex", mood.getSex());
                bundlePeople.putString("userPic", mood.getUserPic());
                bundlePeople.putString("nickName", mood.getNickName());
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(TheApp.instance, ActivityMoodInPut.class);
                intent.putExtra(Config.IsInMoodInput, true);
    ...
    private void LoadingDataFromDatabase() {
        List<Mood> mList = greenUtils.queryMoodList(context,++page, referenceMoodId,Config.DATABASE_MOOD_TYPE_IN);
    ...
    protected void lazyLoad() {//已经初始化完成，并且可见，并且第一次加载才  运行加载.
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                moodList = greenUtils.queryMoodList(context,page, referenceMoodId,Config.DATABASE_MOOD_TYPE_IN);
                mHasLoadedOnce = true;
    ...
    @Subscribe
    public void onEventMainThread(RefleshSchoolInMoodFragment event) {
        String msg = "RefleshSchoolInMoodFragment";
        Log.d("eventbus", msg);
        if (NetUtils.isConnected(TheApp.instance)) {
